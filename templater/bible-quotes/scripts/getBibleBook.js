...
module.exports = async function getBibleBook(tp) {
  ...
  //  --- Detect section and book from path ---
  let filePath = tp.file.path(true).toLowerCase();
  let section = "";
  let detectedBook = null;

  // Check if any book tag is in the path (bookTag followed by a slash)
  for (let sec in books) {
    for (let book of books[sec]) {
      if (filePath.includes(`${book[1]}/`)) {
        section = sec;
        detectedBook = book;
        break;
      }
    }
    if (detectedBook) break;
  }

  // If no book found, detect section first (nt/ot) or ask user
  if (!detectedBook) {
    if (filePath.includes("new-testament/")) {
      section = "nt";
    } else if (filePath.includes("old-testament/")) {
      // ask if apocrypha
      let isApo = await tp.system.suggester(
        ["Yes — apocrypha/deuterocanonical", "No — standard OT"],
        [true, false],
        false,
        "Is the book apocryphal or deuterocanonical?"
      );
      section = isApo ? "apo" : "ot";
    } else {
  ...
  if (detectedBook) {
    bookName = detectedBook[0];
    bookTag = detectedBook[1];
    bookAbbr = detectedBook[2];
  } else {
